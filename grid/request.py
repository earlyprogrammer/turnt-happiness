...
class Occgrid(Request):
    def __init__(self, index):
        super(Occgrid, self).__init__()
        return self.ex("occgrid", str(index))
    
    def corner(self):
        return map(int, self.list[0][1].split(","))

    def grid(self):
        return [[int(i) for i in x[0]] for x in self.list[2:]]
